...
public class RSAUtil {
    ...
    public static PublicKey getPublicKey(String pubKeyBase64) {
        if (StringUtils.isEmpty(pubKeyBase64)) {
            return null;
        }
    ...
    public static PrivateKey getPrivateKey(String priKeyBase64) {
        if (StringUtils.isEmpty(priKeyBase64)) {
            return null;
        }
    ...
    public static byte[] encrypt(byte[] plainData, PublicKey pubKey) {
        if (plainData == null || plainData.length < 1 || pubKey == null) {
            return null;
        }
        try {
            // 获取指定算法的密码器
    ...
    public static byte[] decrypt(byte[] cipherData, PrivateKey priKey) {
        if (cipherData == null || cipherData.length < 1 || priKey == null) {
            return null;
        }
        try {
            // 获取指定算法的密码器
